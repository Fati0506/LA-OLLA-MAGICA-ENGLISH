...
function enviarPedido() {
  ...
  if (typeof globalThis.guardarPedidoEnFirebase === "function") {
    const detalleString = carrito.map(item => `${item.qty}x ${item.nombre}`).join(", ");
    const totalNum = carrito.reduce((sum, item) => sum + item.precio * item.qty, 0);
    
    globalThis.guardarPedidoEnFirebase({
      cliente: nombreCliente,
      tipoEntrega: tipoEntrega,
      direccion: tipoEntrega === 'Delivery' ? direccion : 'Retiro en Local',
      itemsDetalle: detalleString,
      total: totalNum
    });
  }
  // ----------------------------------------------
